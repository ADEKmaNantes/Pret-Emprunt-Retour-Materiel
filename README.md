...
écritures. Toutes les requêtes (lecture ET écriture) passent par `doGet` —
c'est volontaire : les requêtes POST envoyées depuis un site externe comme
GitHub Pages peuvent échouer à cause d'une redirection Google mal lue par
le navigateur. `doPost` reste présent par compatibilité mais n'est plus
