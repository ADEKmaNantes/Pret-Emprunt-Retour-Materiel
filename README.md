...
`Code.gs` est fourni pour référence : c'est le script côté Google Sheets
qui reçoit les lectures/écritures. Il n'a pas besoin d'être dans le dépôt
GitHub — il vit uniquement dans le projet Apps Script associé à la feuille.
Garde-le de côté si tu dois un jour recréer le déploiement Apps Script.
